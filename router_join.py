...
async def get_page_with_features_by_id(page_id: int):
    pool = await get_pool()
...
async def route_neuroroute(page_id: int):
    ...
    try:
        page_row = await get_page_with_features_by_id(page_id)

        if page_row is None:
            raise HTTPException(status_code=404, detail="page_id not found")

        features = {
            "wikitext_length_bytes": page_row["wikitext_length_bytes"],
            "template_count": page_row["template_count"],
            "image_count": page_row["image_count"],
            "reference_count": page_row["reference_count"],
            "heading_count": page_row["heading_count"],
            "internal_link_count": page_row["internal_link_count"],
            "external_link_count": page_row["external_link_count"],
            "category_count": page_row["category_count"],
        }
