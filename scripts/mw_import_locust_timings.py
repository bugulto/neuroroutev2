...
INPUT_PATH = os.path.join("loadtests", "results", "mw_sequential_response_times.csv")
...
def load_timings() -> Tuple[int, int, List[Tuple[int, float]]]:
    ...
    averages = [(page_id, statistics.mean(values)) for page_id, values in timings.items()]
    ...
    try:
        with conn:
            with conn.cursor() as cursor:
                ...
                slow_count, fast_count = update_is_slow(cursor, p80)
    finally:
        conn.close()
